...
def call_gemini_api(prompt, system_message=None):
    ...
    try:
        ...
        response = requests.post(
            f"{url}?key={GOOGLE_API_KEY}", json=payload, headers=headers, timeout=30
        )
...
def generate_chat_response(history, user_message):
    ...
    if not location and current_location:
        if any(
            ...
            for kw in ["attractions", "places", "visit", "see", "things to do"]
        ):
...
def api_upload_document():
    ...
    try:
        ...
        return jsonify(
            {"success": True, "filename": filename, "size": file_size, "status": status}
        )
...
def api_delete_document(filename):
    try:
        ...
        return jsonify({"success": True, "filename": filename, "status": status})
...
def api_refresh_knowledge_base():
    try:
        status = load_knowledge_base()
        return jsonify(
            {"success": True, "status": status, "files_loaded": len(knowledge_base)}
        )
    except Exception as e:
        return jsonify({"error": str(e)}), 500
